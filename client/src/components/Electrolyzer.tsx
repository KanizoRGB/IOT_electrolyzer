...
export default function Dashboard() {
  ...
  const getParameterStatus = (value: number, parameter: string): ParameterStatus => {
    const thresholds: Record<string, { warning: [number, number], critical: [number, number] }> = {
      ...
      // NEW ADDS
      waterLevel: { warning: [30, 100], critical: [0, 30] },
      chamberTemperature: { warning: [70, 85], critical: [85, 100] },
      oxygenRate: { warning: [0, 5], critical: [0, 2] },
      hydrogenRate: { warning: [0, 5], critical: [0, 2] },
      setProductionRate: { warning: [80, 100], critical: [0, 80] },
    };
  ...
  const getTrend = (parameter: string): TrendDirection => {
    ...
    const values = recent.map(reading => {
      switch (parameter) {
        ...
        case 'waterLevel': return reading.waterLevel;
        case 'chamberTemperature': return reading.chamberTemperature;
        case 'oxygenRate': return reading.oxygenRate;
        case 'hydrogenRate': return reading.hydrogenRate;
        case 'setProductionRate': return reading.setProductionRate;
        default: return 0;      }
    });
  ...
  const getCurrentParameters = (): Record<string, ParameterData> => {
    if (!currentData) {
      return {
        ...
        current: { value: 40, trend: "stable", status: "normal" },
        temperature: { value: 25, trend: "stable", status: "normal" },
        pressure: { value: 117, trend: "stable", status: "normal" },
        pH: { value: 50, trend: "stable", status: "normal" },
        efficiency: { value: 20, trend: "stable", status: "normal" }
      };
    }
  ...
  //       value: (() => {
  //         switch (parameter) {
  //           case 'voltage': return reading.voltage ?? 20;
  //           case 'current': return reading.current ?? 40;
  //           case 'temperature': return reading.temperature ?? 25;
  //           case 'pressure': return reading.pressure ?? 117;
  //           case 'pH': return reading.pH ?? 50;
  //           case 'efficiency': return reading.efficiency ?? 20;
  //           default: return 0;
  //         }
  //       })()
  //     }));
  ...
  // };
const generateChartData = (parameter: string) => {
  const history = historicalChartData.length
    ? historicalChartData
    : (historicalData.get(selectedSystemId) || []);

  // if no data at all, return static fallback
  if (!history.length) {
    return Array.from({ length: 10 }).map((_, i) => ({
      time: `${i}:00`,  // fake time
      value: (() => {
        switch (parameter) {
          // case 'voltage': return 20 + i;
          // case 'current': return 40 + i;
          // case 'temperature': return 25 + i;
          // case 'pressure': return 117 + i;
          // case 'pH': return 50 + i;
          // case 'efficiency': return 20 + i;
          // default: return 0;
          case 'voltage': return 20 + Math.sin(i / 2) * 5 + Math.random() * 2;  // around 20
          case 'current': return 40 + Math.cos(i / 3) * 10 + Math.random() * 3; // around 40
          case 'temperature': return 25 + Math.sin(i / 4) * 3 + Math.random() * 1.5; // around 25
          case 'pressure': return 117 + Math.cos(i / 5) * 8 + Math.random() * 4; // around 117
          case 'pH': return 50 + Math.sin(i / 6) * 7 + Math.random() * 2; // around 50
          case 'efficiency': return 20 + Math.cos(i / 2.5) * 6 + Math.random() * 1.5; // around 20
          default: return 0;
        }
      })()
    }));
  ...
  return history.slice(-20).map((reading: any, i: number) => ({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg;grid-cols-3 gap-6 mb-8">
        <ParameterCard
  title="Water Level"
  value={currentData?.waterLevel ?? 700}
  unit="%"
  trend="stable"
  status="normal"
...
<ParameterCard
  ...
  value={currentData?.chamberTemperature ?? 25}
  unit="°C"
  trend="stable"
  status="normal"
...
<ParameterCard
  title="O₂ Production"
  value={currentData?.oxygenRate ?? 45}
  unit="L/min"
  trend="stable"
  status="normal"
...
<ParameterCard
  title="H₂ Production"
  value={currentData?.hydrogenRate ?? 60}
  unit="L/min"
  trend="stable"
  status="normal"
  icon={<Activity className="h-4 w-4" />}
/>
<ParameterCard
  title="Set Production Rate"
  value={currentData?.setProductionRate ?? 70}
  unit="%"
  trend="stable"
  status="normal"
...
/>
<ParameterCard
  title="Electrolyzer Status"
  value={currentData?.electrolyzerStatus === "normal" ? 1 : 0}
  unit=""
  trend="stable"
  status={currentData?.electrolyzerStatus === "normal" ? "normal" : "warning"}
