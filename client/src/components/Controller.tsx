...
export default function Dashboard() {
  ...
  const getParameterStatus = (value: number, parameter: string): ParameterStatus => {
    const thresholds: Record<string, { warning: [number, number], critical: [number, number] }> = {
      ...
      controllerTemperature: { warning: [40, 60], critical: [60, 80] }, // °C
      signalStrength: { warning: [2, 4], critical: [0, 2] },             // bars or scale (0–5)
      batteryLevel: { warning: [30, 60], critical: [0, 30] },            // %
      creditBalance: { warning: [20, 50], critical: [0, 20] } 
};
  ...
  const getTrend = (parameter: string): TrendDirection => {
    ...
    const values = recent.map(reading => {
      switch (parameter) {
        ...
        case 'controllerTemperature': return reading.controllerTemperature ?? 0;
        case 'signalStrength': return reading.signalStrength ?? 0;
        case 'batteryLevel': return reading.batteryLevel ?? 0;
        case 'creditBalance': return reading.creditBalance ?? 0;
        default: return 0;
      }
    });
...
const getCurrentParameters = (): Record<string, ParameterData> => {
  if (!currentData) {
    return {
      voltage: { value: 20, trend: "stable", status: "normal" },
      current: { value: 10, trend: "stable", status: "normal" },
      temperature: { value: 25, trend: "stable", status: "normal" },
      pressure: { value: 117, trend: "stable", status: "normal" },
      pH: { value: 5, trend: "stable", status: "normal" },
      efficiency: { value: 50, trend: "stable", status: "normal" },
      controllerTemperature: { value: 40, trend: "stable", status: "normal" },
      signalStrength: { value: 50, trend: "stable", status: "normal" },
      batteryLevel: { value: 30, trend: "stable", status: "normal" },
      creditBalance: { value: 256, trend: "stable", status: "normal" },
      controllerStatus: {
        value: 0,   
        trend: "stable",
        status: "critical"
  ...
  return {
     controllerStatus: {
        value: currentData.controllerStatus === "normal" ? 1 : 0,
        trend: "stable", // no trend for status
        status: currentData.controllerStatus === "normal" ? "normal" : "critical",
    },
    voltage: {
    ...
    //   value: currentData.controllerTemperature,
    //   trend: getTrend('controllerTemperature'),
    //   status: getParameterStatus(currentData.controllerTemperature, 'controllerTemperature')
    // },
    controllerTemperature: {
  value: currentData.controllerTemperature ?? 0,
  trend: getTrend('controllerTemperature'),
  status: getParameterStatus(currentData.controllerTemperature ?? 0, 'controllerTemperature')
},

    signalStrength: {
      value: currentData.signalStrength ?? 0,
      trend: getTrend('signalStrength'),
      status: getParameterStatus(currentData.signalStrength ?? 0, 'signalStrength')
    },
    batteryLevel: {
      value: currentData.batteryLevel ?? 0,
      trend: getTrend('batteryLevel'),
      status: getParameterStatus(currentData.batteryLevel ?? 0, 'batteryLevel')
    },
    creditBalance: {
      value: currentData.creditBalance ?? 0,
      trend: getTrend('creditBalance'),
      status: getParameterStatus(currentData.creditBalance ?? 0, 'creditBalance')
    },
  };
};

const getControllerStatus = (data: LiveSensorData | null): "normal" | "abnormal" | "stable" => {
  if (!data) return "abnormal";

  const { rtcStatus, gsmStatus, batteryLevel } = data;

  const rtcOk = rtcStatus === "ok";
  const gsmOk = gsmStatus === "ok";
  const batteryOk = (batteryLevel ?? 0) >= 30;

  // Add your own logic for "stable" if needed, otherwise "normal"/"abnormal"
  return rtcOk && gsmOk && batteryOk ? "normal" : "abnormal";
};

// Add WiFi status helper
const getWiFiStatus = (data: LiveSensorData | null): "available" | "unavailable" => {
  if (!data) return "unavailable";

  if ("wifiStatus" in data) {
    return (data as any).wifiStatus === "available" ? "available" : "unavailable";
  }
  return (data.signalStrength ?? 0) > 0 ? "available" : "unavailable";
};
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Badge
          variant={getControllerStatus(currentData) === "normal" ? "default" : "destructive"}
          className="text-sm"
        >
          Controller Status: {getControllerStatus(currentData)}
        </Badge>
          <Badge
          variant={getWiFiStatus(currentData) === "available" ? "default" : "secondary"}
          className="text-sm"
        >
          Wi-Fi: {getWiFiStatus(currentData)}
        </Badge>
</div>
...
<input
  ...
  id="exampleDataList"
  placeholder="AI prompt for analyzing system data..."
  style={{ color: "black" ,width: "700px", height:"40px", borderRadius:"8px",paddingLeft:"10px"}}
/>
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
  ...
  value={currentParameters.controllerTemperature.value}
  unit="°C"
  trend={currentParameters.controllerTemperature.trend}
  status={currentParameters.controllerTemperature.status}
  icon={<Thermometer className="h-4 w-4" />}
/>
<ParameterCard
  title="Signal Strength"
  value={currentParameters.signalStrength.value}
  unit="/5"
  trend={currentParameters.signalStrength.trend}
  status={currentParameters.signalStrength.status}
  icon={<Wifi className="h-4 w-4" />}
/>
<ParameterCard
  title="Battery Level"
  value={currentParameters.batteryLevel.value}
  unit="%"
  trend={currentParameters.batteryLevel.trend}
  status={currentParameters.batteryLevel.status}
  icon={<Power className="h-4 w-4" />}
/>
<ParameterCard
  title="Credit Balance"
  value={currentParameters.creditBalance.value}
  unit="units"
  trend={currentParameters.creditBalance.trend}
  status={currentParameters.creditBalance.status}
...
/>
  ...
  value={currentParameters.controllerStatus.value}
  unit="units"  // "normal" or "abnormal"
  trend={currentParameters.controllerStatus.trend}
  status={currentParameters.controllerStatus.status}
  icon={<Activity className="h-4 w-4" />}
/> */}
<ParameterCard
  title="Controller Status"
  value={currentParameters.controllerStatus.value}
  unit={currentParameters.controllerStatus.value === 1 ? "normal" : "abnormal"}
  trend={currentParameters.controllerStatus.trend}
  status={currentParameters.controllerStatus.status}
  icon={<Activity className="h-4 w-4" />}
/>
...
/>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            ...
            <Button variant="outline" data-testid="button-maintenance">
             ...
             <Button variant={currentData?.mode === "automatic" ? "default" : "secondary"} >
              <RefreshCcw className="h-4 w-4 mr-2" />
               Mode: {currentData?.mode === "automatic" ? "Automatic" : "Manual"}
            </Button>
          </div>
        </CardContent>
      </Card>
  ...
  <CardContent>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Badge variant={currentData?.status === "on" ? "default" : "secondary"}>
        Status: {currentData?.status === "on" ? "On" : "Off"}
      </Badge>
      <Badge variant={currentData?.mode === "automatic" ? "default" : "secondary"}>
        Mode: {currentData?.mode === "automatic" ? "Automatic" : "Manual"}
      </Badge>
    </div>
  </CardContent>
